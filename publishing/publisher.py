...
def _dry_run_publish(image_path: Path, caption: str, alt_text: str | None = None) -> str:
    out_dir = config.GENERATED_IMAGES_DIR / "dry_run"
    out_dir.mkdir(parents=True, exist_ok=True)
    stem = image_path.stem
    text = caption if not alt_text else f"{caption}\n\n--- ALT TEXT ---\n{alt_text}"
    (out_dir / f"{stem}.caption.txt").write_text(text, encoding="utf-8")
...
def publish(image_path: Path, caption: str, *, alt_text: str | None = None, max_retries: int = 3) -> str:
    """Publish a photo post. Returns the FB post id (or a dryrun_ id in DRY_RUN).

    alt_text (if given) is set as the photo's custom alt text — indexed by Google
    since July 2025, so it roughly doubles the searchable text per post.
    """
    ...
    if config.DRY_RUN:
        return _dry_run_publish(image_path, caption, alt_text)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if alt_text:
                data["alt_text_custom"] = alt_text[:1000]
